    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          <div className="flex items-center space-x-2 text-rose-600 dark:text-rose-400 text-xs font-semibold uppercase tracking-wider mb-1">
            ...
            <span>UN SDG 3 - Good Health & Emergency Response Gateway</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold">Healthcare, Emergency & Free Rural Medical Camps</h1>
          <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 mt-1">
            Access 24/7 emergency response, free public clinics, and upcoming rural eye, cardiology & nutrition health camps.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-2 p-1.5 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
          ...
          >
            ...
            <span>Free Rural Medical Camps</span>
          </button>
      ...
      <div className="p-6 rounded-3xl bg-red-950 text-white-force border-2 border-red-500/80 shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <span className="text-[11px] font-black uppercase tracking-widest text-red-200 text-white-force">EMERGENCY MEDICAL HELPLINE (24 HOURS)</span>
            <h3 className="text-xl sm:text-2xl font-black text-white text-white-force mt-0.5">Call 108 Emergency Ambulance Services</h3>
            <p className="text-xs text-red-100 text-white-force mt-0.5 font-medium">Toll-free emergency dispatch with ALS trauma support in &lt;10 minutes.</p>
          </div>
        </div>
        ...
        >
          ...
          <span className="text-white text-white-force">Call 108 Emergency Now</span>
        </a>
      </div>
        ...
        <div className="p-6 rounded-3xl bg-emerald-950 text-white-force border-2 border-emerald-500 text-white space-y-4 animate-in fade-in">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="h-10 w-10 rounded-2xl bg-emerald-500 text-slate-950 flex items-center justify-center font-bold text-xl">
                ✓
              </div>
              <div>
          ...
          <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-white-force">
            <div>
              ...
              <span className="font-semibold text-emerald-400">{bookedCampTicket.date} ({bookedCampTicket.time})</span>
            </div>
            <div>
              <span className="text-slate-400 block">Venue Address:</span>
              <span className="font-semibold text-slate-200">{bookedCampTicket.venue}</span>
            </div>
            <div>
              <span className="text-slate-400 block">Patient Registered:</span>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold flex items-center space-x-2">
              ...
              <span>Upcoming Free Medical, Eye & Cardiology Camps in Rural Districts</span>
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {camps.map((camp) => (
              <div key={camp.id} className="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-slate-800 hover:border-rose-500/40 transition-all flex flex-col justify-between space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400">{camp.availableSlots} Slots Available</span>
                  </div>

                  <h3 className="text-base font-bold mb-1">{camp.title}</h3>
                  ...
                  <div className="p-3 rounded-2xl bg-slate-100/90 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 space-y-2 text-xs">
                    ...
                    <div className="pt-2 border-t border-slate-200 dark:border-slate-900">
                      <span className="text-slate-500 dark:text-slate-400 block text-[10px] font-semibold mb-1">Specialties & Diagnostics:</span>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {SAMPLE_HEALTHCARE_SERVICES.map((srv) => (
              <div key={srv.id} className="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-4">
                <div className="flex items-center justify-between">
                ...
                <div>
                  <h3 className="text-base font-bold mb-1">{srv.name}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">{srv.type} • {srv.distance}</p>
                  <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed mb-3">{srv.address}</p>
                </div>
                ...
                <div className="p-3 rounded-2xl bg-slate-100/90 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 space-y-1 text-xs">
                  ...
                  {srv.services.map((s, idx) => (
                    <div key={idx} className="flex items-center space-x-1 text-slate-800 dark:text-slate-200 text-[11px]">
